...
class ProjectTool(Toolkit):
    ...
    def __init__(self):
        super().__init__(
            name="project_tool",
            instructions=[
                "Use this tool to create new project folders",
                "Project names are automatically sanitized for filesystem compatibility",
                "Projects are created in an 'output' directory",
                "Only one project can be active at a time"
            ]
        )
        self.register(self.create_project)
        self._active_project_folder: Optional[str] = None
    ...
    def get_active_project_folder(self) -> Optional[str]:
        """
